    ...
    Boolean MarkCoins(string denomination, int number)
    {
        ...
        foreach (string file in files)
        {
            ...
            if (x > 0 && !filename.Contains("forsale"))
            {
                ...
                if (sub == denomination)
                {
                    string newfilename = HttpRuntime.AppDomainAppPath.ToString() + @"\" + path + @"\bank\" + filename.Insert(filename.LastIndexOf("."),"forsale");
    ...
    Boolean UnMarkCoins(string denomination, int number)
    {
        ...
        foreach (string file in files)
        {
            ...
            if (filename.Contains("forsale") && x > 0)
            {
                ...
                if (sub == denomination)
                {
                    string newfilename = HttpRuntime.AppDomainAppPath.ToString() + @"\" + path + @"\bank\" + filename.Replace("forsale", "");
                    System.IO.File.Move(file, newfilename);
                    x = x - 1;
                }
            }
